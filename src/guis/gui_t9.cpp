...
create_tab_content_t9(lv_obj_t* tab) {
    ...
    static lv_coord_t row_dsc[] = {50, 50, 50, 50, LV_GRID_TEMPLATE_LAST}; // manual y distribution to compress the grid a bit
    ...
    for (int i = 0; i < 12; i++) {
        ...
        lv_obj_set_user_data(obj,
                             (void*) (intptr_t) i); // Add user data so we can identify which button caused the container event
        // } else {
