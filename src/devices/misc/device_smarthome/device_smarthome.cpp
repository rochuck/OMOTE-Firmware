...
register_device_smarthome() {
    ...
    register_command(&SMARTHOME_MQTT_BULB1_SET,
                     makeCommandData(MQTT, {"bulb1_set"})); // payload must be set when calling commandHandler
    register_command(&SMARTHOME_MQTT_BULB2_SET,
                     makeCommandData(MQTT, {"bulb2_set"})); // payload must be set when calling commandHandler
    register_command(&SMARTHOME_MQTT_BULB1_BRIGHTNESS_SET,
                     makeCommandData(MQTT, {"bulb1_setbrightness"})); // payload must be set when calling commandHandler
