...
// init WiFi - needs to be after init_gui() because WifiLabel must be available
    ...
    // In Windows/Linux there is no loop function that is automatically being called. So we have to do this on our own infinitely
    // here in main()
    unsigned long IMUTaskTimer      = 0;
    unsigned long updateStatusTimer = 0;
    while (1) loop(&IMUTaskTimer, &updateStatusTimer);
#endif
}
...
loop(unsigned long* pIMUTaskTimer, unsigned long* pUpdateStatusTimer) {
    ...
    if (get_irReceiverEnabled()) { infraredReceiver_loop(); }
    // update LVGL UI
    gui_loop();
