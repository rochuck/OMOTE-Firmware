...
// This include of "hardwareLayer.h" is the one and only link to folder "hardware". The file "hardwareLayer.h" does the
// differentiation between ESP32 and Windows/Linux. "hardwareLayer.h" includes all the other hardware header files as well. So
// everything from all hardware header files is available here - and only here. This include has to be here in
// "hardwarePresenter.cpp" and not in "hardwarePresenter.h". Otherwise the whole rest of the code would have access to the hardware
// too. The rest of the code is only allowed to use "hardwarePresenter.h".
...
// Holds the raw keystates as received from the keypad (OMOTE_HARDWARE_REV <= 4), the TCA8418 (OMOTE_HARDWARE_REV >= 5) or the
// simulator. We expect only IDLE_PRESSED and IDLE_RELEASED, because this is what all three sources can deliver (only the keypad
// could also deliver IDLE and HOLD) The whole array is passed as a pointer to the hardware implementation, which fills it with the
// raw key states.
...
void
getKeys(rawKey rawKeys[keypadROWS][keypadCOLS], unsigned long currentMillis) {
    // we need to provide currentMillis to the hardware, because at least in case of the simulator there is no way to access
    // millis()
