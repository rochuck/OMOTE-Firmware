...
register_keyboardCommands() {
    ...
    // But you have to change keys.cpp, gui_numpad.cpp and commandHandler.cpp where keyboard commands are used so that a command can
    // be executed successfully. Search for "executeCommand(Key" to find them.
...
std::string
convertStringListToString(std::list<std::string> listOfStrings) {
    std::string result;
    for (const auto& word : listOfStrings) { result += word + ","; }
    return result;
}
...
executeCommand(uint16_t command, std::string additionalPayload) {
    ...
    } catch (const std::out_of_range& oor) { omote_log_e("executeCommand: internal error, command not registered\r\n"); }
}
...
receiveWiFiConnected_cb(bool connected) {
    ...
    if (connected) {
        ...
        // When receiving this message, your home automation software could send the states of the smart home devices known to
        // OMOTE. With that, OMOTE could show on startup the correct status of the smart home devices.
        //
        // Remark: in your home automation software, maybe add a short delay (e.g. 100-200 ms) between receiving this message and
        // sending out the status of the smart home devices. WiFi connection could be already available, but MQTT connection could
        // be not completely ready. Just try what works for you.
