...
// Read the keypad
void
my_keypad_read(lv_indev_drv_t* indev_driver, lv_indev_data_t* data) {

    // if the coommand handler has queued a key, we want to set it
    // for lvgl
    // the timeout is at zero before send.
    if (queued_key && !queued_key_timeout) {
        data->key          = queued_key; // From your software callback
        data->state        = LV_INDEV_STATE_PRESSED;
        queued_key_timeout = 10;
        printf("my_keypad_read: got key '%d', will send to lvgl, set timeout to %d\r\n", queued_key, queued_key_timeout);
        queued_key         = 0;
        queued_key_timeout = 0;
    }
    // when we have counted down to 1, send the release
    if (queued_key && queued_key_timeout == 1) {
        data->key   = queued_key; // From your software callback
        data->state = LV_INDEV_STATE_RELEASED;
    }
    if (queued_key_timeout) { queued_key_timeout--; }
...
// https://bugs.launchpad.net/ubuntu/+source/libsdl2/+bug/1775067/comments/7
    ...
    indev_drv_mouse.read_cb =
        sdl_mouse_read; /*This function will be called periodically (by the library) to get the mouse position and state*/
    ...
    // indev_drv_keyboard.read_cb = sdl_keyboard_read;  /*This function will be called periodically (by the library) to get the
    // keyboard events*/ lv_indev_t *keyboard_device = lv_indev_drv_register( &indev_drv_keyboard ); lv_group_t *group =
    // lv_group_create(); lv_indev_set_group(keyboard_device, group); lv_group_add_obj(group, lv_scr_act()); lv_group_add_obj(group,
    // tabview); lv_group_add_obj(group, lv_tabview_get_content(tabview)); lv_group_add_obj(group, tabs);
    //
    // need to be in a loop
    // printf("last key: %d\n",lv_indev_get_key(keyboard_device));

    // Initialize the keypad driver
    static lv_indev_drv_t kpindev_drv;
    lv_indev_drv_init(&kpindev_drv);
    kpindev_drv.type    = LV_INDEV_TYPE_KEYPAD;
    kpindev_drv.read_cb = my_keypad_read;
    indev_keypad        = lv_indev_drv_register(&kpindev_drv);
