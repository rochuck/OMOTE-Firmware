...
const uint8_t LCD_BL_GPIO   = 21;
const uint8_t LCD_EN_GPIO   = 10;
const uint8_t LCD_CS_GPIO   = 15;
const uint8_t LCD_DC_GPIO   = 2;
const uint8_t LCD_MOSI_GPIO = 13;
const uint8_t LCD_SCK_GPIO  = 14;
#else
...
init_tft(void) {
    ...
    if (err != ESP_OK) { Serial.println("Error when calling ledc_timer_config!"); }
...
#else
    Serial.println(
        "Will immediately charge VSW voltage. If screen is completely bright, with no content, then this is the reason.");
...
void
update_backlightBrightness_HAL(void) {
    // A variable declared static inside a function is visible only inside that function, exists only once (not created/destroyed
    // for each call) and is permanent. It is in a sense a private global variable.
...
void
set_backlightBrightness_HAL(uint8_t aBacklightBrightness) {
